...
def test_plot_functions(sample_data, monkeypatch):
    ...
    assert isinstance(job_title_counts, pd.Series)
    assert isinstance(experience_level_counts, pd.Series)


def test_calculate_salary_stats_by_experience(sample_data):
    result = calculate_salary_stats_by_experience(sample_data)
    assert isinstance(result, pd.DataFrame)
    assert result.index.tolist() == ["EN", "EX", "MI", "SE"]
    assert result.columns.levels[0].tolist() == ["salary_in_usd"]
    assert result.columns.levels[1].tolist() == [
        "count",
        "mean",
        "median",
        "min",
